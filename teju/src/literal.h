...
#define teju_literal2(a1, a0) (                   \
    ((teju_u1_t) (a1) << (1u * teju_width / 2u)) | \
    ((teju_u1_t) (a0) << (0u * teju_width / 2u))   \
  )

/**
 * @brief Constructs a number from its four quarters.
 *
 * Let N = teju_width / 4. Given a_3, a_2, a_1, a_0 in [0, 2^N[, this
 * macro expands to:
 *   a_3 * 2^(6 * N) + a_2 * 2^(4 * N) + a_1 * 2^N + a_0.
 *
 * Note: here 2^N denotes 2 raised to N. (Similarly for other exponents.)
 *
 * For instance, if teju_width == 128, then:
 * @code{.cpp}
...
#define teju_literal4(a3, a2, a1, a0) (            \
    ((teju_u1_t) (a3) << (3u * teju_width / 4u)) | \
    ((teju_u1_t) (a2) << (2u * teju_width / 4u)) | \
    ((teju_u1_t) (a1) << (1u * teju_width / 4u)) | \
    ((teju_u1_t) (a0) << (0u * teju_width / 4u))   \
  )
